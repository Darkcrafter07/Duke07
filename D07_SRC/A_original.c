//ORIGINAL A.ASM C-CODE COUNTERPART BY KEN SILVERMAN
//Summary:
//2. A.C is not an exact replacement of A.ASM. I also simplified
//some things in ENGINE.C. For example, the biggest thing that comes
//to mind is: vlineasm4 and mvlineasm4 have been removed. These routines
//are "optimized" versions of vlineasm1 and mvlineasm1, and they 
//don't provide any extra functionality to the engine.
//3. KVX voxel support is missing! I was a bit selfish at the time and
//thought this feature should only exist in the PC version. It was
//probably a mistake. :> This only affects the port of Shadow Warrior
// to the Mac.
//4. ksqrt() and krecip() are C replacements of nsqrtasm() and krecipasm()
//(respectively). You should be warned that my movement code relies
//on these functions, so if you use them, and you play a network game
//(or record a demo) with the original version, it will go out of sync
//pretty fast. If this is important for your project, then you must use
//(or simulate) the lookup table I use in my original assembler code.
//
//-Ken S.
