...
// Darkcrafter07 mod of Ken version.
// Allows to fill pixels to left (-) and right (+) to pixelize if some columns were skipped
void slopevlin2pixelizedC(long p, long i, long slopaloffs, long cnt, long bx, long by)
{
    ...
    while (cnt > 0)
    {
        ...
        ((char *)p)[0] = color;
        // for xskip = 2
        ((char *)p)[1] = color; // write 1px right
        // for xskip = 3
        // ((char *)p)[2] = color; // write 2px right
...
grouscan_nonfpu (long dax1, long dax2, long sectnum, char dastat)
{
	...
	for(x=dax1;x<=dax2;x++)
	{
		...
		if (y1 <= y2)
		{
			...
			globaly3 = (globaly2>>10);
			asm3 = mulscale16(y2,globalzd) + (globalzx>>6);
			slopevlin2(ylookup[y2]+x+frameoffset,asm3,(long)nptr2,y2-y1+1,globalx1,globaly1);
