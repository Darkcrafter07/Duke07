    ...
    for (k = (numgroupfiles-1); k >= 0; k--)
    {
        if (groupfil[k] != -1)
        {
            for (i = 0; i < gnumfiles[k]; i++)
            {
                ...
                for (filenum = 0; filenum < MAXOPENFILES; filenum++)
                {
                    if (filehan[filenum] == -1)
                    {
                        filehan[filenum] = i;
                        filegrp[filenum] = k;
                        filepos[filenum] = 0;
...
long kread_uncached(long handle, void *buffer, long leng)
{
    ...
    if ((groupnum < 0) || (groupnum >= numgroupfiles))
    {
        return -1;
    }
    if (groupfil[groupnum] != -1)
    {
        ...
        size = gfileoffs[groupnum][filenum+1]-gfileoffs[groupnum][filenum]-filepos[handle];
        if (leng > size) leng = size;
        readleng = read(groupfil[groupnum], buffer, leng);
        if (readleng > 0)
        {
            filepos[handle] += readleng;
            groupfilpos[groupnum] += readleng;
        }
        return readleng;
    }
