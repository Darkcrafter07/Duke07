...
long reciptabletextureint[4096], reciptableshadeint[4096];

char britable[16][64];
char textfont[1024], smalltextfont[1024];

// so why this "getkensmessagecrc" was coded for pentium?
// bswap is 586 instruction so we code it to work on 386
// and comment out this bs away from the code base where
// each "nice try" occurences, otherwise it quits game
...
extern long setupslopevlin2(long,long,long);
...
extern long setupslopevlinLQX(long,long,long);
#pragma aux setupslopevlinLQX parm [eax][ebx][ecx] modify [edx];
extern long slopevlinLQX(long,long,long,long,long,long);
#pragma aux slopevlinLQX parm [eax][ebx][ecx][edx][esi][edi];
extern long setupslopevlin2LQX(long,long,long);
#pragma aux setupslopevlin2LQX parm [eax][ebx][ecx] modify [edx];
extern long slopevlin2LQX(long,long,long,long,long,long);
#pragma aux slopevlin2LQX parm [eax][ebx][ecx][edx][esi][edi];
...
// good for perspective correction
#pragma aux krecipasmtextureint =\
	"cdq",\
	"mov ecx, eax",\
	"test eax, eax",\
	"jns short krtxtintpos",\
	"neg eax",\
		"krtxtintpos:",\
	"cmp eax, 4095",\
	"jbe short krtxtinttable",\
	"bsr ebx, eax",\
	"push esi",\
	"mov esi, ebx",\
	"mov ecx, ebx",\
	...
	"and ebx, 4095",\
	"push edx",\
	"mov eax, [reciptabletextureint + ebx*4]",\
	"mov ebx, eax",\
	"mov eax, [esp+4]",\
	"imul ebx",\
	"shrd eax, edx, 30",\
	...
	"shr eax, cl",\
	"pop esi",\
	"jmp short krtxtintfin",\
		"krtxtinttable:",\
	"test eax, eax",\
	"jz short krtxtint0z",\
	"mov eax, [reciptabletextureint + eax*4]",\
		"krtxtintfin:",\
	"xor eax, edx",\
		"krtxtintdone:",\
	"jmp short krtxtintend",\
		"krtxtint0z:",\
	"mov eax, 7FFFFFFFh",\
		"krtxtintend:",\
	parm [eax]\
...
// good for dimmed light shading
#pragma aux krecipasmshadeint =\
	"cdq",\
	"mov ecx, eax",\
	"test eax, eax",\
	"jns short krshdintpos",\
	"neg eax",\
		"krshdintpos:",\
	"cmp eax, 4095",\
	"jbe short krshdinttable",\
	"bsr ebx, eax",\
	"push esi",\
	"mov esi, ebx",\
	"mov ecx, ebx",\
	...
	"and ebx, 4095",\
	"push edx",\
	"mov eax, [reciptableshadeint + ebx*4]",\
	"mov ebx, eax",\
	"mov eax, [esp+4]",\
	"imul ebx",\
	"shrd eax, edx, 30",\
	...
	"shr eax, cl",\
	"pop esi",\
	"jmp short krshdintfin",\
		"krshdinttable:",\
	"test eax, eax",\
	"jz short krshdint0z",\
	"mov eax, [reciptableshadeint + eax*4]",\
		"krshdintfin:",\
	"xor eax, edx",\
		"krshdintdone:",\
	"jmp short krshdintend",\
		"krshdint0z:",\
	"mov eax, 7FFFFFFFh",\
		"krshdintend:",\
	parm [eax]\
			 ...
			 short daang, long dahoriz, short dacursectnum)
{
	...
	if (inpreparemirror)
	{
		...
		if (dist_slow == 0)
		{
			// use original drawalls without rendering distant objects slower optimization
			drawalls(0L);
		}
		else
		{
			// use optimized drawalls by rendering distant objects slower according to ticker
			drawalls_distslow(0L);
		}
...
{
    long i, j, ox, oy, x, y1, y2, twall, bwall;
    long portalfovfactor; // Local factor for portal zoom sync
    sectortype *sec;
    ...
    // --- CEILING PORTAL FOV ZOOM START (LQ2X version) ---
    // DO: Maintain visual consistency between walls and low-quality floors.
    if (rendermodeportal && portalzoomadj != 0) 
    {
        // Factor should be the same as in high-quality scan.
        portalfovfactor = divscale16(65536, 65536 + (portalzoomadj >> 1)); 
        globalx1 = mulscale16(globalx1, portalfovfactor);
        globalx2 = mulscale16(globalx2, portalfovfactor);
        globaly1 = mulscale16(globaly1, portalfovfactor);
...
florscan_LQ2X (long x1, long x2, long sectnum)
{
    long i, j, ox, oy, x, y1, y2, twall, bwall;
    long portalfovfactor; // Local scale factor for portal zoom sync
    sectortype *sec;
    ...
    // --- FLOOR PORTAL FOV ZOOM START (LQ2X version) ---
    // Synchronize LQ floor scale with the portal's zoomed FOV.
    if (rendermodeportal && portalzoomadj != 0) 
    {
        // Scale the texture vectors before coordinate system transformation.
        portalfovfactor = divscale16(65536, 65536 + (portalzoomadj >> 1)); 
        globalx1 = mulscale16(globalx1, portalfovfactor);
        globalx2 = mulscale16(globalx2, portalfovfactor);
        globaly1 = mulscale16(globaly1, portalfovfactor);
...
wallscan_LQ2X(long x1, long x2, short *uwal, short *dwal, long *swal, long *lwal)
{
    long i, x, xnice, ynice, fpalookup, shade;
    long y1ve[4], y2ve[4], u4, d4, dax, z, tsizx, tsizy;
    long xstep, x_effective, x_base, j, current_x, x_index, next_x, skip;
    char bad;
    ...
    // Low Detail Implementation Start
    xstep = pxlzefctr; // Use pxlzefctr directly

    x = x1;
    while ((x <= x2) && (((x + frameoffset) & 3) != 0))
    {
        if ((x - x1) % xstep != 0)
        {
            x++;
    ...
    x_base = x; // save to x, so we don't have to save to a temporary variable

    while (x_base <= x2 - 3)
    {
        // Apply low detail skip to the base x, other skip will be handled further
        if ((x_base - x1) % xstep != 0)
        {
            if (pxlzefctr == 1)
            {
                x_base+=4;
            ...
            {
                next_x = x1 + ((x_base - x1) / xstep + 1) * xstep;
                skip = next_x - x_base;
                x_base+=skip;
            }
        }
        else
        {
            bad = 0;
            z=3;
            while(z >= 0)  // for (z = 3; z >= 0; z--)
            {
                current_x = x_base + z;

                y1ve[z] = max(uwal[current_x], umost[current_x]);
                y2ve[z] = min(dwal[current_x], dmost[current_x]) - 1;
                ...
                {
                    i = lwal[current_x] + globalxpanning;
                    if (i >= tsizx)
                    {
                        if (xnice == 0)
                        {
                    ...
                    vince[z] = swal[current_x] * globalyscale;
                    vplce[z] = globalzd + vince[z] * (y1ve[z] - globalhoriz + 1);
                }
                z--;
            }
            ...
            {
                palookupoffse[0] = fpalookup + (getpalookup((long)mulscale16(swal[x_base], globvis), globalshade) << 8);
                palookupoffse[3] = fpalookup + (getpalookup((long)mulscale16(swal[x_base + 3], globvis), globalshade) << 8);
                ...
                {
                    palookupoffse[1] = fpalookup + (getpalookup((long)mulscale16(swal[x_base + 1], globvis), globalshade) << 8);
                    palookupoffse[2] = fpalookup + (getpalookup((long)mulscale16(swal[x_base + 2], globvis), globalshade) << 8);
                }
                ...
                if ((bad != 0) || (u4 >= d4))
                {
                    if (!(bad & 1)) prevlineasm1(vince[0], palookupoffse[0], y2ve[0] - y1ve[0], vplce[0], bufplce[0], ylookup[y1ve[0]] + x_base + frameoffset + 0);
                    if (!(bad & 2)) prevlineasm1(vince[1], palookupoffse[1], y2ve[1] - y1ve[1], vplce[1], bufplce[1], ylookup[y1ve[1]] + x_base + frameoffset + 1);
                    if (!(bad & 4)) prevlineasm1(vince[2], palookupoffse[2], y2ve[2] - y1ve[2], vplce[2], bufplce[2], ylookup[y1ve[2]] + x_base + frameoffset + 2);
                    if (!(bad & 8)) prevlineasm1(vince[3], palookupoffse[3], y2ve[3] - y1ve[3], vplce[3], bufplce[3], ylookup[y1ve[3]] + x_base + frameoffset + 3);

                }
                else
                {
                    if (u4 > y1ve[0]) vplce[0] = prevlineasm1(vince[0], palookupoffse[0], u4 - y1ve[0] - 1, vplce[0], bufplce[0], ylookup[y1ve[0]] + x_base + frameoffset + 0);
                    if (u4 > y1ve[1]) vplce[1] = prevlineasm1(vince[1], palookupoffse[1], u4 - y1ve[1] - 1, vplce[1], bufplce[1], ylookup[y1ve[1]] + x_base + frameoffset + 1);
                    if (u4 > y1ve[2]) vplce[2] = prevlineasm1(vince[2], palookupoffse[2], u4 - y1ve[2] - 1, vplce[2], bufplce[2], ylookup[y1ve[2]] + x_base + frameoffset + 2);
                    if (u4 > y1ve[3]) vplce[3] = prevlineasm1(vince[3], palookupoffse[3], u4 - y1ve[3] - 1, vplce[3], bufplce[3], ylookup[y1ve[3]] + x_base + frameoffset + 3);

                    if (d4 >= u4) vlineasm4(d4 - u4 + 1, ylookup[u4] + x_base + frameoffset);

                    i = x_base + frameoffset + ylookup[d4 + 1];
                    if (y2ve[0] > d4) prevlineasm1(vince[0], palookupoffse[0], y2ve[0] - d4 - 1, vplce[0], bufplce[0], i + 0);
                    if (y2ve[1] > d4) prevlineasm1(vince[1], palookupoffse[1], y2ve[1] - d4 - 1, vplce[1], bufplce[1], i + 1);
                    if (y2ve[2] > d4) prevlineasm1(vince[2], palookupoffse[2], y2ve[2] - d4 - 1, vplce[2], bufplce[2], i + 2);
                    if (y2ve[3] > d4) prevlineasm1(vince[3], palookupoffse[3], y2ve[3] - d4 - 1, vplce[3], bufplce[3], i + 3);
        ...
        while (x_base <= x2)
        {
            ...
            {
                y1ve[0] = max(uwal[x_base], umost[x_base]);
                y2ve[0] = min(dwal[x_base], dmost[x_base]);
                ...
                else
                {
                    ...
                    bufplce[0] = lwal[x_base] + globalxpanning;
                    if (bufplce[0] >= tsizx)
                    {
                        if (xnice == 0)
                        {
                    ...
                    vince[0] = swal[x_base] * globalyscale;
                    vplce[0] = globalzd + vince[0] * (y1ve[0] - globalhoriz + 1);

                    vlineasm1(vince[0], palookupoffse[0], y2ve[0] - y1ve[0] - 1, vplce[0], bufplce[0] + waloff[globalpicnum], x_base + frameoffset + ylookup[y1ve[0]]);
                    x_base++;
                }
            }
        }
...
maskwallscan_LQ2X(long x1, long x2, short *uwal, short *dwal, long *swal, long *lwal)
{
    long i, x, startx, xnice, ynice, fpalookup, shade;
    long y1ve[4], y2ve[4], u4, d4, dax, z, p, tsizx, tsizy;
    long xstep, x_effective, x_base, j, current_x, x_index;
    char bad;
    ...
    for (x = x1; x <= x2; ++x)
    {
        ...
        {
            p++;
            continue; // Skip this pixel for low detail
        }

        x_effective = x; // Use the initial x value for all calculations within the step

        y1ve[0] = max(uwal[x_effective], startumost[x_effective + windowx1] - windowy1);
        y2ve[0] = min(dwal[x_effective], startdmost[x_effective + windowx1] - windowy1);
        if (y2ve[0] <= y1ve[0])
        {
            p++;
            continue;
        ...
        palookupoffse[0] = fpalookup + (getpalookup((long)mulscale16(swal[x_effective], globvis), globalshade) << 8);

        bufplce[0] = lwal[x_effective] + globalxpanning;
        if (bufplce[0] >= tsizx)
        {
            if (xnice == 0)
                bufplce[0] %= tsizx;
        ...
        vince[0] = swal[x_effective] * globalyscale;
        vplce[0] = globalzd + vince[0] * (y1ve[0] - globalhoriz + 1);
...
transmaskwallscan_LQ2X(long x1, long x2)
{
    long x, startx;
    long xstep, x_effective, x_base, j, current_x, x_index;
    ...
    // Apply low-detail interleaving AND transparency handling
    for (x = x1; x <= x2; )
    {
        if ((x - x1) % xstep != 0)
        {
        ...
        {
            transmaskvline2_LQ2X(x);
            x += 2;
            if ((x - x1) % xstep != 0)
            {
                x-=1; //only advance by 1 when stepping over the end of a xstep block
            }
        }
        else if (x <= x2)
        {
...
genreciptabletextureint()
{
    ...
    for(i = 0; i < 4096; i++)
    {
        if (i == 0) reciptabletextureint[i] = 0x7FFFFFFF;
        else reciptabletextureint[i] = 1073741824L / i;
    }
}
...
genreciptableshadeint()
{
    ...
    for(i = 0; i < 4096; i++)
    {
        if (i == 0) reciptableshadeint[i] = 0x7FFFFFFF;
        else reciptableshadeint[i] = 2147483647L / i;
    }
}
		...
		case 200:
			for(i=permtail;i!=permhead;i=((i+1)&(MAXPERMS-1)))
			{
				...
				if ((per->pagesleft > 0) && (per->pagesleft <= numpages))
				{
					...
					{
					//use high definiton hud-sprites drawing function if lqmode is off
...
krecipint(long num)
{
	return(krecipasmtextureint(num));
}
...
drawmaskwall(short damaskwallcnt)
{
	...
	if ((globalorientation&128) == 0)
		...
		if ( (use_lqmode) == 1 )
		{
		maskwallscan_LQ2X(xb1[z],xb2[z],uwall,dwall,swall,lwall);
		}
		else if ( (use_lqmode) == 0 )
		{
	...
	else
	{
		...
		if ( (use_lqmode) == 1 )
		{
		transmaskwallscan_LQ2X(xb1[z],xb2[z]);
		}
		else if ( (use_lqmode) == 0 )
		{
...
drawsprite_LQ2X (long snum)
{
	...
	if ((cstat&48) == 0)
	{
		...
		// Low detail mode - skip some calculations - improved
		detail_counter = 0;
		for(x=lx;x<=rx;x++)
		{
			if (detail_counter % pxlzefctr == 0) {
				uwall[x] = max(startumost[x+windowx1]-windowy1,(short)startum);
				dwall[x] = min(startdmost[x+windowx1]-windowy1,(short)startdm);
			} else {
				// Carry forward previous values to prevent uninitialized gaps
				uwall[x] = (x > lx) ? uwall[x-1] : max(startumost[x+windowx1]-windowy1,(short)startum);
				dwall[x] = (x > lx) ? dwall[x-1] : min(startdmost[x+windowx1]-windowy1,(short)startdm);
			}
			detail_counter++;
		}
		...
		if ((cstat&2) == 0)
			maskwallscan_LQ2X(lx,rx,uwall,dwall,swall,lwall);
		else
			transmaskwallscan_LQ2X(lx,rx);
	}
	else if ((cstat&48) == 16)
	{
		if ((cstat&4) > 0) xoff = -xoff;
		...
		for(i=xb1[MAXWALLSB-1];i<=xb2[MAXWALLSB-1];i++)
		{
			if (detail_counter % pxlzefctr == 0) {
				...
				swall[i] = (i > xb1[MAXWALLSB-1]) ? swall[i-1] : (krecipasm(hplc)<<2);
			}
			hplc += hinc;
			detail_counter++;
		}
		...
		for(i=smostwallcnt-1;i>=0;i--)
		{
			...
			if (max(yb1[MAXWALLSB-1],yb2[MAXWALLSB-1]) > min(yb1[j],yb2[j]))
			{
				...
				else
				{
					...
					z1 = (xp2-xp1)*(y1-yp1) - (yp2-yp1)*(x1-xp1);
					z2 = (xp2-xp1)*(y2-yp1) - (yp2-yp1)*(x2-xp1);
					if ((z1^z2) >= 0)  //If wall's NOT facing you
						x = (z1+z2);
					else
					{
						z1 = (x2-x1)*(yp1-y1) - (y2-y1)*(xp1-x1);
	...
	else if ((cstat&48) == 32)
	{
		...
		for(x=lx;x<=rx;x++)
		{
			if (detail_counter % pxlzefctr == 0) {
				...
				uwall[x] = (x > lx) ? uwall[x-1] : max(uwall[x],startumost[x+windowx1]-windowy1);
				dwall[x] = (x > lx) ? dwall[x-1] : min(dwall[x],startdmost[x+windowx1]-windowy1);
			}
			detail_counter++;
		}
...
ceilspritescan (long x1, long x2)
{
	...
	while (y1 < y2 - 1)
	{
	  ++y1;
		...
		{
		//use high definiton sprite-ceiling drawing routine if lqmode is off
...
ceilspritescan_LQ2X(long x1, long x2)
{
    long x, y1, y2, twall, bwall;
    long ystart;
    ...
    for (x = x1; x <= x2; x++)
    {
        ...
        if (twall < bwall - 1)
        {
            ...
            {
                while (y1 < y2 - 1) {
                    if (y1 % pxlzefctr == 0) // Use y1 for stable vertical skipping
            ...
            while (y2 > bwall) {
                if ((y2 - 1) % pxlzefctr == 0) // Align with downward direction
...
ceilspritehline (long x2, long y)
{
	...
	//x = x1 + (x2-x1)t + (y1-y2)u  і  x = 160v
	//y = y1 + (y2-y1)t + (x2-x1)u  і  y = (scrx-160)v
	//z = z1 = z2                   і  z = posz + (scry-horiz)v
...
ceilspritehlinelq (long x2, long y)
{
    ...
    if (pxlzefctr_counter % pxlzefctr != 0)  // Skip based on pxlzefctr
    {
        pxlzefctr_counter++; // Increment even when skipping
        return;
    }
    ...
    else
    {
     ...
     pxlzefctr_counter++; // Increment every time
}
...
//===============================================================================================================
// dorotatesprite functions START
//===============================================================================================================
...
dorotatespritelq (long sx, long sy, long z, short a, short picnum, signed char dashade, char dapalnum, char dastat, long cx1, long cy1, long cx2, long cy2)
{
	...
	// Low detail mode - skip some calculations
	detail_counter = 0;
	for(x= (lx>>16); x<= (rx>>16); x++) {
		if (detail_counter % pxlzefctr != 0) { // Skip every pxlzefctr - 1 iterations
			detail_counter++;
			continue;
		}
...
grouscanfpu_LQ2X (long dax1, long dax2, long sectnum, char dastat)
{
	...
	setupslopevlinLQX(((long)(picsiz[globalpicnum]&15))+(((long)(picsiz[globalpicnum]>>4))<<8),waloff[globalpicnum],-ylookup[1]);
	...
	// ORIGINAL: for(x=dax1;x<=dax2;x++)
	for(x=dax1;x<=dax2;x += pxlzefctr) // Iterate over pixelization factor to skip some columns and later fill by "mov [ebp+1], al"
	{
		...
		if (y1 <= y2)
		{
			...
			slopevlinLQX(ylookup[y2]+x+frameoffset,krecipasm(asm3>>3),(long)nptr2,y2-y1+1,globalx1,globaly1);

			if ((x&15) == 0) faketimerhandler();
		}
		globalx2 += globalx * pxlzefctr;
		globaly2 += globaly * pxlzefctr;
		globalzx += globalz * pxlzefctr;
		shoffs += shinc * pxlzefctr;
	}
}

// === integer slopes start ===
long bpl, transmode = 0;
long glogx, glogy, gbxinc, gbyinc, gpinc;
char *gbuf, *gpal, *ghlinepal, *gtrans;

// Ken's original port of asm "setupslopevlin"
setupslopevlin2C(long logylogx, long bufplc, long pinc)
{
	glogx = (logylogx&255); glogy = (logylogx>>8);
	gbuf = (char *)bufplc; gpinc = pinc;
}

// Darkcrafter07 mod of Ken version, faithful to the original one. Uses absolute persp calc.
// Allows to fill pixels to left(-), right(+) to pixelize if columns were skipped. Slow.
void slopevlin2pixelizedC(long p, long i, long slopaloffs, long cnt, long bx, long by)
{
    long *slopalptr = (long *)slopaloffs;
    long bz = asm3;
    long bzinc = (asm1 >> 3);
    unsigned long u, v;
    char color;
    ...
    while (cnt > 0)
    {
        // i = krecip(bz>>6); // the fpu krecipasm
        i = krecipint(bz>>6); // the int krecipasm (almost identical)
        bz += bzinc;
        ...
        // 3. Sample color (gbuf and palette)
        color = *(char *)(slopalptr[0] + gbuf[((u >> (32 - glogx)) << glogy) + (v >> (32 - glogy))]);

        ((char *)p)[0] = color;
        // for xskip = 2
        ((char *)p)[1] = color;    // write 1px right (for LQ2X)
        // for xskip = 3
        // ((char *)p)[2] = color; // write 2px right (for LQ3X)
...
// Want to use C version? This one is the BEST - has an optimized asm "slopevlin2" counterpart. 
// Darkcrafter07 version of Ken "slopevlin2C" but using the same Ken's way to calculate the
// perspective relatively, like in original asm "slopevlin", dramatically increasing performance.
// Does so by calculating krecip only each 8px and then interpolating du-dv between relatively.
void slopevlin2relativeC(long p, long start_z, long slopaloffs, long cnt, long bx, long by)
{
    long bz, bzinc, bz_next, i, k, i_next, di;
    long du, dv, step;
    long ush, vsh, gy, pinc;
    unsigned long u, v;
    char color, **slptr, *gb, *d;

    slptr = (char **)slopaloffs;
    ush = (32 - glogx); vsh = (32 - glogy); gy = glogy;
    pinc = gpinc; gb = gbuf;

    bz = start_z; 
    bzinc = (asm1 >> 3); // Depth step

    i = krecipint(bz >> 6); // Perspective correction
    u = bx + globalx3 * i;
    v = by + globaly3 * i;
    ...
    {
        step = (cnt >= 8) ? 8 : cnt;
        if (step <= 0) break; // crash protect from div0

        bz_next = bz + bzinc * step;
        i_next = krecipint(bz_next >> 6);
        di = i_next - i;

        // Scale coordinates to prevent mirroring effect after certain distance
        if (step == 8)
        {
            // mulscale3 does (a*b) >> 3 via 64-bit intermediate result
            du = mulscale3(globalx3, di); 
            dv = mulscale3(globaly3, di);
        }
        else 
        {
            // Use regular div for the tail but with mulscale(..., 0)
            // to save the sign and 64-bit mul precision
            du = mulscale(globalx3, di, 0) / step;
            dv = mulscale(globaly3, di, 0) / step;
        }

        d = (char *)p;

        #define drwslpintrel8stp                                   \
        {                                                          \
            color = (*slptr)[gb[((u >> ush) << gy) + (v >> vsh)]]; \
            *d = color;                                            \
            u += du; v += dv; slptr--; d += pinc;                  \
        }

        if (step == 8)
        {
            drwslpintrel8stp; drwslpintrel8stp; drwslpintrel8stp; drwslpintrel8stp;
            drwslpintrel8stp; drwslpintrel8stp; drwslpintrel8stp; drwslpintrel8stp;
        } 
        else 
        {
            k = step;
            while (k--) drwslpintrel8stp;
        }
        #undef drwslpintrel8stp

        bz = bz_next; i = i_next; p = (long)d;
        cnt -= step;
    }
}
...
grouscanint_LQ2X (long dax1, long dax2, long sectnum, char dastat)
{
	...
	wx = wall[wal->point2].x - wal->x;
	wy = wall[wal->point2].y - wal->y;
	//dasqr = krecipasmtextureint(nsqrtasm(wx*wx+wy*wy));
	//i = mulscale21(daslope,dasqr);
	dasqr = fastinvsqrt(wx*wx+wy*wy);
	i = mulscale21(daslope,dasqr);
	wx *= i; wy *= i;
	...
	setupslopevlin2LQX(((long)(picsiz[globalpicnum]&15))+(((long)(picsiz[globalpicnum]>>4))<<8),waloff[globalpicnum],-ylookup[1]);
	...
	// find out if the whole sloped surface (sector) is far
	perspdepthdist = krecipasmtextureint(klabs(daz) >> 8);
	if (perspdepthdist < 48000) slopeisfar = 1; else slopeisfar = 0;
	...
	for(x=dax1;x<=dax2;x += pxlzefctr) // Iterate over pixelization factor to skip some columns and later fill by "mov [ebp+1], al"
	{
		...
		if (y1 <= y2)
		{
		        //nptr1 = (long *)&slopalookup[y1+(shoffs>>15)];
		        //nptr2 = (long *)&slopalookup[y2+(shoffs>>15)];
		        //prolong lightdim (needs even closer distances to brighten)
		        nptr1 = (long *)&slopalookup[y1+(shoffs>>15)-(shoffs>>28)];
			...
			globaly3 = (globaly2>>10);
			asm3 = mulscale16(y2,globalzd) + (globalzx>>6);
			slopevlin2LQX(ylookup[y2]+x+frameoffset,asm3,(long)nptr2,y2-y1+1,globalx1,globaly1);
...
parascan_LQ2X (long dax1, long dax2, long sectnum, char dastat, long bunch)
{
	sectortype *sec;
	long i, j, k, l, m, n, x, y, z, wallnum, nextsectnum, globalhorizbak;
	long scale_factor, scaled_ang, skyfov;
	short *topptr, *botptr;
	...
	//overall x-axis image stretch correction
	k = 11 - (picsiz[globalpicnum]&15) - pskybits;
	x = -1;
	...
	for(z=bunchfirst[bunch];z>=0;z=p2[z])
	{
		...
		if ((nextsectnum < 0) || (wall[wallnum].cstat&32) || ((j&1) == 0))
		{
			...
			// parallaxtype 0 - simple horizontal image x-axis scroll
			if (parallaxtype == 0)
			{
				//n = mulscale16(xdimenrecip,viewingrange);
				n = mulscale16(xdimenrecip,skyfov);
			...
			// parallaxtype 1 - doomish horizontal stretch to the sides
			else if (parallaxtype == 1)
			{
				for(j=xb1[z];j<=xb2[z];j++)
					lplc[j] = ((((long)radarang2[j]+globalang)&2047)>>k);
			}
			// parallaxtype 2 - cylindical image x-axis scroll
			...
			// parallaxtype 3 - doomish horizontal wider
			if (parallaxtype == 3)
			{
				scale_factor = 45875;  // 0.7 in 16.16 fixed-point format
				for(j = xb1[z]; j <= xb2[z]; j++)
				{
				    scaled_ang = ((long)radarang2[j] * scale_factor) >> 16;
				    lplc[j] = ((scaled_ang + globalang) & 2047) >> k;
				}
			}
			else
				//clearbuf((long)(&swplc[xb1[z]]),xb2[z]-xb1[z]+1,mulscale16(xdimscale,viewingrange));
		...
		else if (x >= 0)
		{
			...
			if (((lplc[x]^lplc[xb1[z]-1])>>m) == 0)
				wallscan_LQ2X(x,xb1[z]-1,topptr,botptr,swplc,lplc);
			else
			{
				j = x;
				while (x < xb1[z])
				{
					...
					if (n != globalpicnum)
					{
						wallscan_LQ2X(j,x-1,topptr,botptr,swplc,lplc);
						j = x;
						globalpicnum = n;
					}
					x++;
	...
	if (x >= 0)
	{
		...
		if (((lplc[x]^lplc[xb2[bunchlast[bunch]]])>>m) == 0)
			wallscan_LQ2X(x,xb2[bunchlast[bunch]],topptr,botptr,swplc,lplc);
		else
		{
			j = x;
			while (x <= xb2[bunchlast[bunch]])
			...
			if (j <= x)
				wallscan_LQ2X(j,x,topptr,botptr,swplc,lplc);
		}
		globalpicnum = l;
	}
	globalhoriz = globalhorizbak;
...
//=======================================================================
// parascan (parallax sky drawing) functions END
//=======================================================================
