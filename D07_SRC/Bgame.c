...
// comment "waitforeverybody" if you use multi instead of mmulti
	waitforeverybody();
...
drawscreen(short snum, long dasmoothratio)
{
	...
	if (dimensionmode[snum] != 2)
	{
		...
		else
		{
			...
			else if (gotpic[SKYCUSTOM1>>3]&(1<<(SKYCUSTOM1&7)))
			{
				gotpic[SKYCUSTOM1>>3] &= ~(1<<(SKYCUSTOM1&7));
				pskyoff[0] = 0; pskyoff[1] = 1; pskyoff[2] = 2; pskyoff[3] = 3;
				pskyoff[4] = 4; pskyoff[5] = 5; pskyoff[6] = 6; pskyoff[7] = 7;
				pskybits = 3;
			}
